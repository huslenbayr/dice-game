...
function buildLeadNotificationHtml({ brandName, lead }) {
  const isInquiry = Boolean(
    lead.fullName || lead.country || lead.travelDates || lead.travelers || lead.tourInterest || lead.message
  );

  return buildBaseEmailLayout({
    brandName,
    title: isInquiry ? "New travel inquiry received" : "New homepage lead captured",
    intro: isInquiry
      ? "A traveler submitted the contact inquiry form and is waiting for follow-up."
      : "A visitor requested more travel details from the homepage email capture form.",
    content: [
      lead.fullName ? paragraph("Full name", lead.fullName) : "",
      paragraph("Email", lead.email),
      lead.country ? paragraph("Country", lead.country) : "",
      lead.travelDates ? paragraph("Travel dates", lead.travelDates) : "",
      lead.travelers ? paragraph("Travelers", lead.travelers) : "",
      lead.tourInterest ? paragraph("Tour interest", lead.tourInterest) : "",
      lead.message ? paragraph("Message", lead.message) : "",
