...
import { capturePayPalOrder } from "@/lib/payments/paypal-service";
import {
...
async function getBookingAndPaymentSession(repository, bookingId, reference = null) {
  const { snapshot, booking, tour } = await getBookingAndTour(repository, bookingId);
  const paymentSession =
    snapshot.paymentSessions.find((session) => {
      if (reference && session.bookingId === bookingId && session.reference === reference) {
        return true;
      }

      return session.bookingId === bookingId;
    }) || null;
...
export async function createPaymentForBooking(payload, options = {}) {
  const { bookingId, method, card, promoCode } = assertPaymentCreatePayload(payload);
  ...
  const { sessionPayload, pricing } = await createPaymentSessionWithProvider({
    booking,
    tour,
    method,
    card,
  ...
  const rawResponse =
    sessionPayload.rawResponse && typeof sessionPayload.rawResponse === "object" && !Array.isArray(sessionPayload.rawResponse)
      ? {
          ...sessionPayload.rawResponse,
          promo: pricing.promo
        }
      : {
          provider: sessionPayload.provider || null,
          data: sessionPayload.rawResponse ?? null,
          promo: pricing.promo
        };
...
export async function capturePayPalCheckout({ bookingId, reference, orderId, mock = false }) {
  const repository = await getRepository();
  const { booking, tour, paymentSession } = await getBookingAndPaymentSession(repository, bookingId, reference);
  const method = inferPaymentMethod(paymentSession);

  if (method !== "paypal") {
    throw new Error("The latest payment session for this booking is not a PayPal checkout.");
  }

  if (reference && paymentSession.reference !== reference) {
    throw new Error("This PayPal session no longer matches the active booking checkout.");
  }
  ...
  if (paymentSession.status === "paid") {
    return buildPaymentResponse({
      booking,
      paymentSession,
      paymentContext: getPaymentCheckoutContext(booking),
      tour,
      alreadyPaid: true
    });
  }
  ...
  const capturedOrder = await capturePayPalOrder({
    orderId: orderId || paymentSession.externalId,
    mock
  });
  const result = await repository.updatePaymentSessionStatus(
    { paymentId: paymentSession.id },
    {
      ...
      providerStatus: capturedOrder.providerStatus,
      externalId: capturedOrder.externalId || paymentSession.externalId,
      rawResponse: capturedOrder.rawResponse,
      failureReason: capturedOrder.failureReason,
      statusSource: "paypal-return",
      lastCallbackAt: new Date().toISOString(),
      previousStatus
    }
  );

  logInfo("paypal_checkout_captured", {
    bookingId: booking.id,
    paymentId: paymentSession.id,
    previousStatus,
    nextStatus: result.paymentSession.status,
    externalId: result.paymentSession.externalId
  });
  ...
  return buildPaymentResponse({
    ...
    tour,
    emailResult,
    alreadyPaid: result.paymentSession.status === "paid" && previousStatus === "paid"
  });
}

export async function cancelPayPalCheckout({ bookingId, reference, orderId }) {
  const repository = await getRepository();
  const { booking, tour, paymentSession } = await getBookingAndPaymentSession(repository, bookingId, reference);
  const method = inferPaymentMethod(paymentSession);
  ...
  const result = await repository.updatePaymentSessionStatus(
    { paymentId: paymentSession.id },
    {
      ...
      externalId: orderId || paymentSession.externalId,
      rawResponse: {
        provider: paymentSession.provider,
        cancelled: true,
        orderId: orderId || paymentSession.externalId
      },
      failureReason: "The traveler cancelled the PayPal checkout before capture.",
      statusSource: "paypal-cancel",
      lastCallbackAt: new Date().toISOString(),
      previousStatus
    }
  );

  logInfo("paypal_checkout_cancelled", {
    bookingId: booking.id,
    paymentId: paymentSession.id,
    previousStatus,
    nextStatus: result.paymentSession.status
