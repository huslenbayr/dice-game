...
import { resolvePromotionalPricing } from "@/lib/payments/promo-code";
import {
...
export async function createPaymentSessionWithProvider({ booking, tour, method, card, origin, promoCode }) {
  const { methodId, paymentContext, provider } = resolvePaymentMethod(booking, method);
  const pricing = resolvePromotionalPricing({
    baseAmount: getBookingAmount(tour, booking),
    promoCode
  });
