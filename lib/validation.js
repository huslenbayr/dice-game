...
export function assertEmailLeadPayload(payload) {
  ...
  const travelDates = String(payload.travelDates || "").trim();
  const tourInterest = String(payload.tourInterest || "").trim();
  const message = String(payload.message || "").trim();
  const travelersRaw = String(payload.travelers || "").trim();
  const travelers = travelersRaw ? Number(travelersRaw) : null;
  const isContactInquiry = source === "contact-inquiry";
  ...
  if (travelersRaw && (!Number.isInteger(travelers) || travelers < 1 || travelers > 20)) {
    throw createHttpError(400, "Travelers must be between 1 and 20.");
  }
